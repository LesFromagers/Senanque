...
2. **The Heisman Park Ledger** (analytics wing, route: `/analytics/heisman-park-ledger`) — every OU football season, 1895–present, ranked by a normalized Power Index. Tagline "Coronation at the Palace on the Prairie." Ships with the 27 hand-verified marquee seasons live; the other ~104 seasons fill in via the pull scripts below, tracked on the dashboard's own manual-review worklist (`/analytics/heisman-park-ledger/gaps`), not hidden. Second to ship.
   ...
   **Data sources — three, each with one job:**
   ...
   - **Wikipedia REST API, second source: "{year} College Football All-America Team" pages** — NCAA-consensus All-Americans for the Talent layer, back to 1889. Separate from the OU season pages above: each season's own page was checked first and found to essentially lack this data (free text present for only 12 of 131 seasons, 0 auto-extracted, 39 more with an unextracted mention), so this dedicated per-year national All-America page is the real source, pulled by `scripts/heisman_ledger/pull_all_americans.py`. Same domain/etiquette as the primary Wikipedia source (real `User-Agent`, ~1–2 req/sec, already allowlisted). Filters to players in the school column marked exactly "Oklahoma" (never "Oklahoma State") and meeting the **NCAA's consensus threshold** — a player recognized by enough of that season's major selector organizations to count as a true consensus pick, not just any one outlet's selection — never every named mention. Confirmed against three real, structurally different page formats by fetching live pages directly (never assumed from a fixture):
     1. A flat table captioned "Consensus All-Americans," no heading (2003-era) — names wrapped in `{{sortname}}` templates (needs explicit template extraction, not generic wikitext-stripping) and rowspan-grouped rows (needs a carry-forward state machine, or later rows in a shared-cell group silently drop out).
     2. `==Consensus All-Americans==` heading followed by a wikitable (1950/1956/1985-era).
     3. No table or heading at all (2008) — per-position bulleted prose, bold name = consensus, trusted **only** when that specific page's own intro text documents the bold convention (never assumed silently for a page that doesn't state it).

     A year whose page doesn't resolve, or resolves but matches none of these three formats, is left out of the pulled data and logged to `gap_report_all_americans.md` — never guessed at zero. A year that resolves with a real, checked zero Oklahoma consensus selections is stored as an explicit empty result, distinct from unresolved.
