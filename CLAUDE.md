...
2. **The Heisman Park Ledger** (analytics wing, route: `/analytics/heisman-park-ledger`) — every OU football season, 1895–present, ranked by a normalized Power Index. Tagline "Coronation at the Palace on the Prairie." Ships with the 27 hand-verified marquee seasons live; the other ~104 seasons fill in via the pull scripts below, tracked on the dashboard's own manual-review worklist (`/analytics/heisman-park-ledger/gaps`), not hidden. Second to ship.
   ...
   **Power Index formula** (`lib/heisman-ledger/power-index.ts`):
   ...
   ```
   - **Performance (50%):** point differential z-score vs. national average that season (5/7 of this layer, ≈71%) + offense/defense efficiency z-scores (1/7 each, ≈14%), each efficiency number tagged with a `data_tier` (1 = SP+/advanced ~2005+, 2 = yards/play ~1950s+, 3 = yards/game vs. national rank pre-1950s, 4 = points-only for the thinnest seasons). See `scripts/heisman_ledger/` for the tier waterfall logic. The point-diff z-score centers on the real NCAA national average for that season (`lib/heisman-ledger/national-baseline.ts`, 1937–present) but scales by OU's own historical spread — the NCAA's record book publishes the national average only, never a cross-team standard deviation for a historical season, and getting a real one would mean pulling every FBS team's own season figures across ~90 years. That's this formula's *defined* method, not a fallback awaiting better data, and isn't flagged per-season. An SRS-style strength-of-schedule component (30% of this layer, in an earlier draft) has been dropped outright rather than left permanently null and flagged: it needs every OU opponent's own full season game log, a second-order pull with no near-term plan to exist. If either of these ever gets built, it's a deliberate re-addition to the formula, not a silent revival.
   - **Accomplishment (35%, capped at 100):** national title, conference title, final AP rank, bowl result — point table in `lib/heisman-ledger/accomplishment-scoring.ts`. National title reads `national_title_claim` free text (its own wording — does it say "consensus"? — is the real signal, not a proxy for one). Conference title and bowl result read structured fields (`conference_champion`, `bowl_name`, `bowl_result`) populated directly by `pull_wikipedia.py`'s infobox extraction, not pattern-matched from prose at scoring time — an earlier free-text version of this had real bugs (a bowl loss recorded as "L 19–55" scoring as a win) that reading a field the pull already classified removes outright. **Bowl result is scored by stage, symmetrically for both win and loss**, not a flat major/other split: title game (BCS NCG / CFP Championship) 40/18, CFP semifinal 25/10, New Year's Six (Rose/Sugar/Orange/Cotton/Fiesta/Peach) else 15/5, other bowl 8/2 — a Fiesta Bowl win outscores a Cheez-It Bowl win, and a national-title-game loss outscores an Independence Bowl loss, which the earlier flat version couldn't express (OU's own history has 4 national-title-game appearances and 4 CFP-semifinal appearances — 7 of those 8 losses were scoring identically to losing the Independence Bowl before this). This can stack additively with the national-title-claim credit above (a title-game win earns both) rather than needing a special case to avoid double-counting — the whole layer's hard 100-point cap absorbs that naturally, same as before. Fiesta and Peach Bowl were also missing from the original major-bowl list entirely (a real bug, not a design choice) — fixed alongside the tiering.
   - **Talent (15%):** Heisman winner/finalists, All-Americans, draft picks by round — point table in `lib/heisman-ledger/talent-scoring.ts`. Heisman winner = 30 points. Heisman finalist (or, pre-1982 before the Heisman Trust named official finalists, a qualifying top vote finish) = 10 points each, capped at 20 total — additive and unconditional, applying whether or not the season also has a winner (winner alone = 30, winner + 2 finalists = 50, two finalists with no winner = 20). The 20-point cap sits well below a win's own 30 on purpose: finalist credit is real — genuine national recognition — but deliberately kept subordinate to actually winning, never a path to matching or exceeding it through volume. Finalist data has no automated source: confirmed by direct investigation that no Wikipedia page (neither a per-year "{year} Heisman Trophy" article, which doesn't exist, nor OU's own season pages, which don't mention finalist status even for well-known near-misses like 2000 or 2016) or NCAA archive carries this; Sports-Reference likely has full voting tables but is never automated per this file's sourcing rules below. It's therefore a small, hand-supplied, presumed-complete list (`data/heisman-ledger/heisman_finalists.csv`, Matt's own knowledge of OU's Heisman history) rather than a partial pull — a year absent from it is a confirmed zero, not an unresolved gap; if a missed season turns up later, add it directly to that CSV. All-Americans are scored from `consensus_all_americans`/`consensus_all_american_count` (NCAA-consensus selections only, per the second Wikipedia source below), not the older free-text `notable_all_americans` column, which was hand-entered prose with no consensus filtering and was confirmed to overcount relative to true consensus status (1950's free-text list carries 4 names; the real NCAA-consensus table has 2). `notable_all_americans` is kept for display only. Draft picks are scored from `draft_picks_r1_2`/`draft_picks_r3_7` (round 1–2 = 6 points each, round 3–7 = 2 points each, uncapped — no cap specified for this component) — see the third Wikipedia source below for how a pick is mapped to a college season and how a same-player duplicate draft entry is resolved.
   ...
   **Data sources:**
   ...
   - **Wikipedia REST API, second source: consensus All-America navbox templates** — NCAA-consensus All-Americans for the Talent layer, back to 1895 (every OU season). Separate from the OU season pages above: each season's own page was checked first and found to essentially lack this data (free text present for only 12 of 131 seasons, 0 auto-extracted, 39 more with an unextracted mention). The real, authoritative source turned out to be `Template:{year} [NCAA Division I-A/NCAA Division I FBS/](omitted pre-1978) College Football Consensus All-Americans` — a curated Wikipedia navbox transcluded on every year's "{year} College Football All-America Team" page, listing exactly that season's real consensus roster with no bold-detection heuristics needed. Found after a Google AI Overview surfaced several OU 1994–2001 names an earlier (bold/heading-only) version of `scripts/heisman_ledger/pull_all_americans.py` had missed; checking the actual primary source confirmed those names really are consensus picks, which led to finding this template. It carries no per-player school, so each entry is cross-referenced against that year's main article body (which lists every selector's picks, any school) to find the player's school — table-derived body entries preferred over bulleted-prose ones on a name collision (a page's own unrelated trivia bullets can coincidentally match the same shape and, unfixed, once silently overwrote a correct school with garbage). A template entry that can't be matched to any school gets one extra fetch of that player's own bio page, checked for an "Oklahoma" mention, before being flagged for manual review rather than silently dropped or guessed in. Same domain/etiquette as the primary source (real `User-Agent`, ~1–2 req/sec, already allowlisted); a three-format body-only fallback (flat captioned table / heading+table / bold-or-marker bulleted prose, the original method) is kept for the rare year the template doesn't resolve, though in practice it resolved for all 131 OU seasons.

     Never fabricates: a year whose page/template doesn't resolve under either method is left out of the pulled data and logged to `gap_report_all_americans.md` — never guessed at zero. A year that resolves with a real, checked zero Oklahoma consensus selections is stored as an explicit empty result, distinct from unresolved.
   - **Wikipedia REST API, third source: "List of Oklahoma Sooners in the NFL draft"** — every OU draft pick, 1936–present, from one page (`scripts/heisman_ledger/pull_draft_picks.py`), not a per-season one like the two sources above, since this list is already organized by draft class. A pick maps to college season `draft_year − 1`: a player can only enter the draft after finishing the season that made them eligible, which holds whether that was a normal senior year, an early declare, or a one-season transfer-in — confirmed against 30 picks across 6 draft classes (including Adrian Peterson's early declare and Jalen Hurts' one-season transfer) by cross-referencing each player's own bio-page infobox `college` field, zero mismatches. **A same-player duplicate draft-table entry is credited once, to the earlier selection** — 11 OU players have two rows on this page, almost all from the AFL/NFL's separate pre-merger drafts (1960–1969, per the page's own intro text); the later entry is dropped entirely rather than double-counted or split across two seasons. Every season is populated (this is one fully-parsed page, not a partial pull) — 0 picks is a real, confirmed zero. Same domain/etiquette as the other two Wikipedia sources. The pull script has a `--verify` flag that re-fetches every pick's own bio page to re-confirm the season mapping (off by default for the bulk run: the mapping is logically guaranteed, not just empirically likely, and re-verifying all ~429 picks individually ran too slowly under Wikipedia's rate limits to be worth it once the rule was already confirmed with zero exceptions).
