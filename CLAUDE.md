...
2. **The Heisman Park Ledger** (analytics wing, route: `/analytics/heisman-park-ledger`) — every OU football season, 1895–present, ranked by a normalized Power Index. Tagline "Coronation at the Palace on the Prairie." Ships with the 27 hand-verified marquee seasons live; the other ~104 seasons fill in via the pull scripts below, tracked on the dashboard's own manual-review worklist (`/analytics/heisman-park-ledger/gaps`), not hidden. Second to ship.
   ...
   **Power Index formula** (`lib/heisman-ledger/power-index.ts`):
   ...
   - **Accomplishment (35%):** national title, conference title, final AP rank, bowl result — point table in `lib/heisman-ledger/accomplishment-scoring.ts`. National title reads `national_title_claim` free text (its own wording — does it say "consensus"? — is the real signal, not a proxy for one). Conference title and bowl result read structured fields (`conference_champion`, `bowl_name`, `bowl_result`) populated directly by `pull_wikipedia.py`'s infobox extraction, not pattern-matched from prose at scoring time — an earlier free-text version of this had real bugs (a bowl loss recorded as "L 19–55" scoring as a win) that reading a field the pull already classified removes outright.
