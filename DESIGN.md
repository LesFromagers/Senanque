...
- ~~The signature visual element~~ — answered and shipped: **the Arcade** (`components/brand/Arcade.tsx`) — a row of Romanesque arches, thin outline, exactly one bay filled Lavender. It's the hub's one shared decorative motif (homepage hero, section markers) and stays that way. Extension, not exception: a spoke may earn exactly one **project-level** hero motif of its own, drawn in the same one-fill-thin-stroke grammar, standing in for the Arcade on that spoke's own pages — the Heisman Park Ledger's stadium mark (below) is the first instance. The Arcade's own doc comment ("the only decorative motif anywhere in the app") needs a follow-up edit to scope that claim to shared/hub chrome now that a project-level exception exists — flagging here so it doesn't silently drift again.
- ~~Icon and imagery style~~ — answered at two levels, both already in code. Functional UI icons: 24px grid, 1.25px stroke, square caps, monochrome `currentColor` (`components/icons/types.ts`). Decorative hero motifs: the Arcade's stricter cousin of that same spec — thin outline, exactly one Lavender fill, no marks or lettering baked into the mark itself. The Ledger's stadium south-facade elevation is the first project-level hero motif built to it; reach for this spec before inventing a new icon style per project.
